...
from capt.cfg import MAX_CAPTCHA, CHAR_SET_LEN, model_path, IMAGE_HEIGHT, IMAGE_WIDTH
from capt.utils import convert2gray, vec2text
import tensorflow as tf

from socketservice.hack_captcha import stringToRGB, crack_captcha_cnn, hack_captcha
...
class MyTCPHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):  #所有请求的交互都是在handle里执行的,
        ...
        while True:
            ...
            date_str=data.decode().replace("\r\n","").strip()
            if date_str == 'bye':
                print("客户端传输完成:",bs64_str)
                iamge_array = stringToRGB(bs64_str)
                predict_text = hack_captcha(iamge_array)
                self.request.sendall(("captcha:"+predict_text).encode())  # sendall是重复调用send.
                break
            bs64_str=bs64_str+date_str
...
if __name__ == "__main__":
    HOST, PORT = "192.168.92.106", 8888 #windows
